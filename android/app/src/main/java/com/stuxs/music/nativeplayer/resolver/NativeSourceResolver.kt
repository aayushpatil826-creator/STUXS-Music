...
sealed class ResolvedSource {
    data class LocalFile(val file: File, val track: NativeTrack, val mimeType: String? = null) : ResolvedSource()
...
class NativeSourceResolver(private val context: Context) {
     ...
     * 1. Downloaded local file in native storage (checked by exact ID, alt ID, and canonical metadata)
    ...
    suspend fun resolveSource(track: NativeTrack): ResolvedSource {
        ...
        val downloadedEntity = downloadRepo.getDownloadedEntity(track)
        if (downloadedEntity != null) {
            val downloadedFile = File(downloadedEntity.localFilePath)
            if (downloadedFile.exists() && downloadedFile.length() >= 10240L) {
                val updatedTrack = track.copy(
                    isDownloaded = true,
                    localFilePath = downloadedFile.absolutePath
                )
                return ResolvedSource.LocalFile(downloadedFile, updatedTrack, downloadedEntity.mimeType)
            }
        }
